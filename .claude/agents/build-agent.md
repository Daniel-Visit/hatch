  ...
  PostToolUse:
    - matcher: 'Write|Edit'
      hooks:
        ...
        - type: command
          command: >-
            uv run $CLAUDE_PROJECT_DIR/.claude/hooks/validators/no_vapid_private_in_client.py
        - type: command
          command: >-
...
1b. **Consult Domain Experts**: If Domain Expert Knowledge was NOT provided in
your prompt, discover available experts by scanning
`ls .claude/commands/experts/*/expertise.yaml`. For each expert whose domain
matches the file you're building (e.g., `database` for DB models/migrations,
`frontend-state` for React/Zustand, `websocket` for WS handlers,
`hooks` for hook scripts, `testing` for test files):

- Read their `expertise.yaml` to understand established patterns, conventions,
  file locations, and design decisions
- Follow these patterns exactly — experts encode accumulated project knowledge
- If expert knowledge WAS provided inline in your prompt, use that directly
