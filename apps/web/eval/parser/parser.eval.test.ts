...
/**
 * Parser eval harness — env-guarded live test.
 *
 ...
 *
 * Pass criterion: >=80% of the 6 seed cases must pass all assertions.
 */
...
describe.skipIf(!LIVE)('parser eval (live)', () => {
  it('passes >= 80% of eval cases', async () => {
    ...
    for (const r of results) {
      ...
      console.log(
        `[${status}] ${r.id} (confidence=${r.details.parserConfidence.toFixed(2)}, extracted=${r.details.extractedFields.length} fields)`,
      );
      if (!r.passed) {
  ...
  }, 120_000); // Generous timeout — 6 cases × one Sonnet call × ~5-10s per call
});
