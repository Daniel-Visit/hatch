...
-- Note: SPEC §4.3 used `tsvector generated always as (...) stored`, but Postgres
-- 14.5 rejects that (to_tsvector is not IMMUTABLE in this context). We use a
-- regular tsvector column populated by the apps_set_slug trigger instead.
...
create table if not exists public.apps (
  ...
  cover_url       text,
  art_kind        text not null default 'pixel',
...
-- BEFORE INSERT/UPDATE: derive slug if missing, populate search_vector, bump updated_at
...
drop trigger if exists apps_before_write on public.apps;
...
-- The TRIGGERs that fire these land in Phase 4 when source tables exist.
-- ---------------------------------------------------------------------------

create or replace function public.bump_likes_count() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  if tg_op = 'INSERT' then update public.apps set likes_count = likes_count + 1 where id = new.app_id;
  elsif tg_op = 'DELETE' then update public.apps set likes_count = greatest(likes_count - 1, 0) where id = old.app_id;
  end if;
  return null;
end $$;
...
language plpgsql security definer set search_path = public as $$
begin
  if tg_op = 'INSERT' then update public.apps set saves_count = saves_count + 1 where id = new.app_id;
  elsif tg_op = 'DELETE' then update public.apps set saves_count = greatest(saves_count - 1, 0) where id = old.app_id;
  end if;
  return null;
end $$;
...
language plpgsql security definer set search_path = public as $$
begin
  if tg_op = 'INSERT' then update public.apps set comments_count = comments_count + 1 where id = new.app_id;
  elsif tg_op = 'DELETE' then update public.apps set comments_count = greatest(comments_count - 1, 0) where id = old.app_id;
  end if;
  return null;
end $$;
...
language plpgsql security definer set search_path = public as $$
begin
  if tg_op = 'INSERT' then update public.apps set remixes_count = remixes_count + 1 where id = new.app_id;
  elsif tg_op = 'DELETE' then update public.apps set remixes_count = greatest(remixes_count - 1, 0) where id = old.app_id;
  end if;
  return null;
end $$;
