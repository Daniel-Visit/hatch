...
# (HATCH-001 fix) before any payload reaches disk.
...
def create_log_entry(input_data: dict) -> dict:
    ...
    Payload strings are passed through ``redact()`` so high-entropy
    bearer tokens (JWTs, PATs, AWS keys) never reach disk.
    """
    return {
