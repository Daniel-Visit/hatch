...
# Local secret redactor — strips Supabase JWTs, PATs, AWS keys, etc.
# from prompts before they touch disk (HATCH-001 fix).
sys.path.insert(0, str(Path(__file__).parent))
from _redact_secrets import redact  # noqa: E402


def log_user_prompt(session_id, input_data):
    """Log user prompt to logs directory (with secret redaction)."""
    # Ensure logs directory exists (use CLAUDE_PROJECT_DIR for stable path resolution)
    ...
    # Append the entire input data (redacted)
    log_data.append(redact(input_data))
...
def manage_session_data(session_id, prompt, name_agent=False):
    ...
    # Ensure sessions directory exists (use CLAUDE_PROJECT_DIR for stable path resolution)
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR", os.getcwd())
    sessions_dir = Path(project_dir) / ".claude" / "data" / "sessions"
    ...
    if name_agent and "agent_name" not in session_data:
        # Resolve LLM util paths relative to this hook file (not CWD)
        hooks_dir = Path(__file__).parent
        ollama_script = str(hooks_dir / "utils" / "llm" / "ollama.py")
        anth_script = str(hooks_dir / "utils" / "llm" / "anth.py")

        # Try Ollama first (preferred)
        try:
            result = subprocess.run(
                ["uv", "run", ollama_script, "--agent-name"],
                capture_output=True,
                text=True,
